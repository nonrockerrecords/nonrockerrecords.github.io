...
function updateSidebarAbilities() {
  const abilities = [
    { id: 'member-soju', minLevel: 2 }, // unlocked after beating Lvl 1
    { id: 'member-dj', minLevel: 3 },   // after Lvl 2
    { id: 'member-statue', minLevel: 4 }, // after Lvl 3
    { id: 'member-nullsoft', minLevel: 5 } // after Lvl 4
  ];
  
  abilities.forEach(ability => {
    const el = document.getElementById(ability.id);
    if (unlockedLevels.includes(ability.minLevel)) {
...
function reshuffleBoard() {
  do {
    fillBoardNoMatches();
  } while (!hasPossibleMove() || findMatches().coordinates.length > 0);
...
function setupDragDrop(wrapper) {
  ...
  wrapper.addEventListener('touchstart', (e) => {
    ...
    selectedTile = wrapper;
  }, { passive: true });
  
  wrapper.addEventListener('touchend', (e) => {
    if (!selectedTile || boardDisabled || isGameOver) return;
...
function processMatchSequence(matchesObj) {
  ...
  spawnScorePopup(sumR / coords.length, sumC / coords.length, matchScore, color);
...
function triggerPowerUps(triggers) {
  ...
  triggers.forEach(trigger => {
    ...
    if (trigger.type === 'soju' || trigger.type === 'kazoo') {
      // Kazoo blast: Row and Column clear
      // Yung Soju level 1 unlock
      if (!unlockedLevels.includes(2)) return; // Lock check
      ...
      // DJ 막걸리 level 2 unlock
      if (!unlockedLevels.includes(3)) return; // Lock check
      ...
      // STATUE.EXE color clear
      // Statue head unlock level 3
      if (!unlockedLevels.includes(4)) return; // Lock check
      
      playSynth('sawtooth', 880, 0.4, 0.15, 110);
      const targetType = PIECE_KEYS[Math.floor(Math.random() * PIECE_KEYS.length)];
      ...
      // NULLSOFT unlock level 4
      if (!unlockedLevels.includes(5)) return; // Lock check
...
function spawnGlitchedPowerups(spawns) {
  spawns.forEach(spawn => {
    // Check player ability unlocks
    let powerType = null;
    if (spawn.len === 4) {
      // 4-match spawns Kazoo/Soju blast if Level 1 is cleared, Didgeridoo explosion if Level 2 is cleared
      if (unlockedLevels.includes(2) && Math.random() < 0.5) {
        powerType = 'soju'; // Yung Soju Power-up
      } else if (unlockedLevels.includes(3)) {
        powerType = 'dj';   // DJ 막걸리 Power-up
      } else {
        powerType = 'soju'; // Fallback basic power
      }
    } else if (spawn.len === 5) {
      // 5-match spawns Statue or Nullsoft bomb if Level 3/4 is cleared
      if (unlockedLevels.includes(4) && Math.random() < 0.5) {
        powerType = 'statue'; // STATUE.EXE Color Clear
      } else if (unlockedLevels.includes(5)) {
        powerType = 'nullsoft'; // NULLSOFT Random Clear
      } else {
        powerType = 'statue';
      }
    }
...
function startMatrixWash() {
  ...
  for (let i = 0; i < columns; i++) {
    matrixStreams[i] = {
      x: i * 14,
      y: Math.random() * -100, // Starts above screen
